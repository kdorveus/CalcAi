...
const MainScreen: React.FC = () => {
      ...
      const handleResult = (event: any) => {
        ...
        if (event.results && event.results.length > 0) {
          ...
          if (Platform.OS === 'android') {
            ...
            setTimeout(() => {
              setBubbles(prev => prev.filter(bubble => bubble.type !== 'calc'));
            }, 2000);
          }
        }
      };
  ...
  // Add reset button to header when keypad is not shown
  const renderHeaderControls = () => {
    // Don't show reset button if keypad is shown or if there are no bubbles
    if (showKeypad || bubbles.length === 0) return null;
    ...
    return (
      <View style={styles.headerControls}>
        <View style={styles.tooltipContainer}>
          ...
          >
            <MaterialIcons name="refresh" size={24} color="#888" style={{ transform: [{ scaleX: -1 }] }} />
          </Pressable>
